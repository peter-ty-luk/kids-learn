...
const Sound = {
  ...
  KEYS: { pickup: 'pickup', match: 'coin', win: 'win', caught: 'crash', door: 'click', drop: 'click' },
  init() {
...
function buildLevel() {
  ...
  for (let owner = 0; owner < np; owner++) {
    set.pairs.forEach((p, pid) => {
      for (const side of ['left', 'right']) {
        ...
        const it = { x: cell[0], y: cell[1], ox: cell[0], oy: cell[1], owner, pairId: pid, side, text: pairText(side === 'left' ? p.left : p.right), matched: false, carried: false };
        items.push(it);
  ...
  const players = [];
  for (let i = 0; i < np; i++) {
    players.push({ i, x: starts[i][0], y: starts[i][1], sx: starts[i][0], sy: starts[i][1], carrying: null, matched: 0, total: nPairs, moveCd: 0, stun: 0, color: P_COLORS[i], exit: { x: exits[i][0], y: exits[i][1] }, done: false });
  }
  ...
  if (MZ.opts.traps) {
    ...
    for (let i = 0; i < nGuards && i < guardFloors.length; i++) { const c = guardFloors[i]; guards.push({ x: c[0], y: c[1], dir: ['up', 'down', 'left', 'right'][Math.floor(Math.random() * 4)] }); }
...
function computeTileSize() {
  const st = MZ.state; const target = Math.min((window.innerWidth || 800) - 28, 680);
  const ts = Math.max(22, Math.floor(target / st.maze.GW));
  st.ts = ts;
  const cv = $('maze-canvas'); cv.width = st.maze.GW * ts; cv.height = st.maze.GH * ts;
}
...
function onEnter(p) {
  ...
  // item interaction (only the player's own colour)
  const here = st.items.find((it) => !it.matched && !it.carried && it.x === p.x && it.y === p.y && it.owner === p.i);
  if (here) {
    if (!p.carrying) { p.carrying = here; here.carried = true; Sound.play('pickup', 0.8); }
    else if (here.pairId === p.carrying.pairId && here.side !== p.carrying.side) {
      here.matched = true; p.carrying.matched = true; p.carrying = null; p.matched++;
      Sound.play('match', 0.85); flash('✅ Matched!');
      if (p.matched === p.total) flash('🚪 All matched — find the exit!');
    } else if (st.hidden) {
      // Hidden-pairs mode: stepping on a non-matching piece flashes its text so
      // you can memorise where it is, but you can't pick it up (carry only one).
      here.revealUntil = performance.now() + 1100; flash('👀 ' + here.text);
    }
  }
  // exit
...
function teleport(pi, x, y) { const p = MZ.state.players[pi]; p.x = x; p.y = y; onEnter(p); if (MZ.state.dark) recomputeFog(); }
...
function checkGuardCatch() {
  ...
  for (const p of st.players) {
    ...
    if (st.guards.some((g) => g.x === p.x && g.y === p.y)) {
      ...
      p.x = p.sx; p.y = p.sy; p.stun = 700; Sound.play('caught', 0.6); flash('👮 Caught! Back to start.');
...
// ---------- render ----------
function drawWrappedText(ctx, text, cx, cy, maxW, maxH) {
...
function render() {
  ...
  const cv = $('maze-canvas'), ctx = cv.getContext('2d'), ts = st.ts, m = st.maze;
  ctx.clearRect(0, 0, cv.width, cv.height);
  // tiles
  for (let y = 0; y < m.GH; y++) for (let x = 0; x < m.GW; x++) {
    const px = x * ts, py = y * ts;
    if (m.g[y][x] === FLOOR) { ctx.fillStyle = '#f4f1ff'; ctx.fillRect(px, py, ts, ts); }
    else { ctx.fillStyle = '#5a4fb0'; ctx.fillRect(px, py, ts, ts); }
  }
  // exits
  for (const p of st.players) {
    const e = p.exit, allDone = p.matched === p.total;
    ctx.fillStyle = allDone ? '#34d399' : '#9aa6bd';
    roundRect(ctx, e.x * ts + ts * 0.12, e.y * ts + ts * 0.12, ts * 0.76, ts * 0.76, 6); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = `${Math.floor(ts * 0.5)}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(allDone ? '🚪' : '🔒', e.x * ts + ts / 2, e.y * ts + ts / 2 + 1);
  }
  // doors
  for (const d of st.doors) { const px = d.x * ts, py = d.y * ts; ctx.fillStyle = d.open ? 'rgba(120,180,255,0.25)' : '#7c5cff'; ctx.fillRect(px + 1, py + 1, ts - 2, ts - 2); if (!d.open) { ctx.fillStyle = '#fff'; ctx.font = `${Math.floor(ts * 0.4)}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('⛔', px + ts / 2, py + ts / 2 + 1); } }
  // switches
  for (const s of st.switches) { if (s.used) continue; ctx.font = `${Math.floor(ts * 0.5)}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('💡', s.x * ts + ts / 2, s.y * ts + ts / 2 + 1); }
  // items
  for (const it of st.items) {
    if (it.matched || it.carried) continue;
    const px = it.x * ts, py = it.y * ts; const col = P_COLORS[it.owner];
    if (itemRevealed(it)) {
      ctx.fillStyle = it.side === 'left' ? '#fff7ec' : '#eefcf3';
      roundRect(ctx, px + 2, py + 2, ts - 4, ts - 4, 7); ctx.fill();
      ctx.lineWidth = 2.5; ctx.strokeStyle = col.edge; ctx.stroke();
      drawWrappedText(ctx, it.text, px + ts / 2, py + ts / 2, ts - 8, ts - 8);
    } else {
      // face-down card: owner-coloured back with a "?"
      ctx.fillStyle = col.soft;
      roundRect(ctx, px + 2, py + 2, ts - 4, ts - 4, 7); ctx.fill();
      ctx.lineWidth = 2.5; ctx.strokeStyle = col.edge; ctx.stroke();
      ctx.fillStyle = col.edge; ctx.font = `700 ${Math.floor(ts * 0.5)}px 'Segoe UI', system-ui, Arial`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText('?', px + ts / 2, py + ts / 2 + 1);
    }
  }
  // guards
  for (const g of st.guards) { const px = g.x * ts, py = g.y * ts; ctx.font = `${Math.floor(ts * 0.6)}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('👮', px + ts / 2, py + ts / 2 + 1); }
  // players
  for (const p of st.players) {
    if (p.done) continue;
    const px = p.x * ts, py = p.y * ts;
    ctx.fillStyle = p.color.body; ctx.strokeStyle = p.color.edge; ctx.lineWidth = 3;
    ctx.beginPath(); ctx.arc(px + ts / 2, py + ts / 2, ts * 0.32, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    if (p.carrying) { ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(px + ts * 0.72, py + ts * 0.28, ts * 0.12, 0, Math.PI * 2); ctx.fill(); }
    if (p.stun > 0) { ctx.fillStyle = '#fde68a'; ctx.font = `${Math.floor(ts * 0.4)}px serif`; ctx.fillText('💫', px + ts / 2, py + ts * 0.2); }
  }
  // fog
  if (st.dark) {
    for (let y = 0; y < m.GH; y++) for (let x = 0; x < m.GW; x++) {
      if (!visibleAt(x, y)) { ctx.fillStyle = '#241d4d'; ctx.fillRect(x * ts, y * ts, ts, ts); }
      else if (!st.visible.has(key2(x, y)) && st.lit.has(key2(x, y))) { ctx.fillStyle = 'rgba(36,29,77,0.38)'; ctx.fillRect(x * ts, y * ts, ts, ts); }
    }
  }
  updateHud();
}
...
function updateHud() {
  const st = MZ.state;
  st.players.forEach((p, i) => {
    const c = $('carry-' + i); if (c) c.innerHTML = p.carrying ? ('✋ ' + escapeText(p.carrying.text)) : '<span class="empty-h">empty-handed</span>';
...
function updateControlsHint() {
  ...
  $('controls-hint').textContent = st.twoP
    ? '🔵 Player 1: Arrow keys + Space to drop  ·  🔴 Player 2: W A S D + Q to drop'
    : 'Move: Arrow keys or W A S D  ·  Space: drop the piece';
}
...
window.addEventListener('keydown', (e) => {
  ...
  keys.add(e.code);
  if (e.code === 'Space') dropItem(st.players[0]);
  if (e.code === 'KeyQ' && st.twoP) dropItem(st.players[1]);
});
...
document.querySelectorAll('#dpad button').forEach((b) => {
  ...
  const press = (on) => { if (dir === 'drop') { if (on) dropItem(MZ.state.players[0]); return; } const code = Object.keys(P1_MOVE).find((k) => P1_MOVE[k] === dir); if (on) keys.add(code); else keys.delete(code); };
...
function startGame() { $('setup').style.display = 'none'; $('game').style.display = 'block'; $('win-overlay').style.display = 'none'; buildLevel(); flash(MZ.state.set.prompt || ''); Sound.startMusic(); }
$('start-btn').addEventListener('click', startGame);
$('quit-btn').addEventListener('click', () => { MZ.state = null; $('game').style.display = 'none'; $('setup').style.display = 'block'; });
$('win-again').addEventListener('click', () => { $('win-overlay').style.display = 'none'; buildLevel(); flash(MZ.state.set.prompt || ''); });
...
MZ.checkCatch = checkGuardCatch; MZ.passable = passable; MZ.revealed = itemRevealed; MZ.start = startGame;
